...
IGHV・IGHJ・CDR3アミノ酸配列を用いて、CDR3の完全一致（LV0）、Levenshtein距離1（LV1）、距離2（LV2）を区別し、一致したユニーククローン数、総リード数、頻度を表示・保存します。照合エンジンは特定の抗原に固定していませんが、現時点の正式なデータベース入力はCoV-AbDab形式CSVです。その他のデータベースは、将来の汎用データベース形式で対応する計画です。
...
- CoV-AbDabを明示する選択ボタンと、将来拡張用に無効状態の「汎用データベース（今後対応）」ボタンを配置
...
「参照…」から検体ファイルを選ぶと、現在の正式対象であるRG社形式またはCPM社形式を自動判定して選択ボタンへ反映します。ファイル名を直接入力した場合、まだ形式が確定していなければ解析開始時または時点追加時に自動判定します。一方、RG社形式またはCPM社形式を手動選択済みの場合は、その選択を優先します。解析前に表示されている形式ボタンを確認してください。

### CPM様式

現時点の正式対象は、現在使用しているCPM社CSV/TSVです。自動判定では`Vseg`、`Jseg`、`CDR3`列を確認し、読込みには次の列を使用します。
...
列名や構造が異なるCSV/Excelは、現時点の正式なCPM様式には含めません。将来作成する変換アプリで標準化し、`汎用形式`から入力する計画です。
...
現時点の正式なデータベース入力はCoV-AbDab形式CSVです。照合キーにはIGHV、IGHJ、CDR3 AAを使用します。
...
データベースには検体レパトアのようなRead数／カウント数は必要ありません。V、J、CDR3 AAの3項目で照合キーを作れます。抗体名、対象抗原、出典などは照合の最小条件ではありませんが、一致配列の意味と由来を確認するため保持を推奨します。CoV-AbDab以外のデータベースは、仕様を確認したうえで将来の汎用データベース形式から入力します。
...
現在の全テストは30件です。3照合方式、RG/CPM読込み、CoV-AbDab読込み、形式選択、汎用形式の誤作動防止、負数・0・正数・小数Day、数値ソート、重複Day停止、各時点と単独解析の同一性、通常／経時Excel構造を検証します。実データKKF103hG・KKF141hGでも経時内の各時点が単独解析と完全一致することを確認しています。過去の実検体基準値と検証時点のテスト数は [検証結果](docs/VALIDATION_RESULTS.md) に記録しています。

## データ保護

アプリは入力検体とDBを読み取り専用で扱い、元ファイルの内容を変更しません。元の検体ファイルやDBファイル全体を結果Excelへ複製することも、GitHubへ自動送信することもありません。ただし、解析結果Excelの`Matched Clones`には、解析結果の監査に必要な一致クローンのV/J/CDR3、Read数、頻度、対応するDB注釈が保存されます。この結果Excelも研究データとして取り扱ってください。検体、DB、論文、生成結果は`.gitignore`の対象です。公開GitHubへ研究データや解析結果を登録しないでください。
