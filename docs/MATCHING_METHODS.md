...
現時点で本プロジェクトに提供されているCPM仕様情報では、V/Jのアレルまで判定せず、`*01`が便宜的に付与される形式とされています。したがって、CPM列の`*01`を生物学的に確定したアレル判定値として利用しません。

旧QASAS方式は入力されたV/J文字列を変更せず完全一致に使用するため、このようなCPMデータとの組合せには原則使用しません。CPMの主解析はKobe Ver 1.0方式、感度解析はCDR3のみ方式とします。提供元の仕様や出力形式が更新された場合は、便宜的アレル付与の有無を改めて確認してください。
...
V/J完全一致はCDR3距離計算より前に行います。V/J候補が0の場合、LV0だけでなくLV1・LV2についてもCDR3 Levenshtein距離は一度も計算されません。したがって全距離が0でも、生物学的陰性とは限りません。

特に、アレルを判定せず`*01`を便宜的に付与するCPM形式では、DB側に同じ文字列がなければ全候補がV/J結合段階で失われます。DBに一律`*01`を補うことも、確認されていないアレル一致を仮定するため行いません。この場合は旧QASAS方式を使用せず、Kobe Ver 1.0方式を選択します。

旧QASAS方式は旧解析の再現用としてこの完全一致挙動を維持します。CPMへの適用上の制限を理由に旧方式の正規化規則を変更しません。
