...
現在の通常解析・経時解析Excelは、保存時に検体とDBのSHA-256、ファイルサイズ、更新時刻を自動計算して `Input QC` または `Sample QC` と `Method` に記録します。さらにGitコミット、作業ツリー状態、QASASソース全体のSHA-256、Python・主要ライブラリ・OSの版も `Method` に記録します。元の検体ファイルやDBファイル全体を結果Excelへ複製することも、GitHubへ自動送信することもありません。ただし、結果Excelの `Matched Clones` には、一致クローンのV/J/CDR3、Read数、頻度、対応するDB注釈が保存されます。結果Excel自体を研究データとして保護してください。
